...
class RankingCycle:
    ...
    def __init__(self, engine, settings) -> None:
        ...
        self._rankings_dir = Path(settings.RANKINGS_DIR)

        self._rankings_dir.mkdir(parents=True, exist_ok=True)
    ...
    def _persist(self, universe: RankedUniverse) -> None:
        ...
        try:
            filename = self._rankings_dir / f"ranking_{universe.cycle_id}.json"
    ...
    def _prune_old_files(self) -> None:
        ...
        try:
            files = sorted(
                self._rankings_dir.glob("ranking_*.json"),
