...
import { ensureAudioContext } from "@/lib/audio-level";
...
export default function ApolloPage() {
  ...
  const busy = sending || voice.isProcessing || voice.isListening;

  // Prime AudioContext on first user gesture (iOS) and warm up TTS voices
  useEffect(() => {
    function unlock() {
      try {
        ensureAudioContext();
  ...
  async function handleSend(e?: React.FormEvent) {
    ...
    try {
      ...
      setTimeout(() => setApolloState("idle"), 900);
    } catch (err) {
  ...
  function handleVoiceStart(e: React.PointerEvent) {
    e.preventDefault();
    if (voice.isListening || voice.isProcessing) return;
    voice.start();
  }
  function handleVoiceEnd(e: React.PointerEvent) {
    e.preventDefault();
    ...
    <div className="mx-auto grid w-full max-w-7xl gap-6 lg:grid-cols-[1.15fr_1fr]">
      ...
      <section className="flex flex-col items-center justify-start gap-6">
        ...
        >
          ...
          STATUS · {STATE_LABEL[apolloState] ?? apolloState}
          {voice.isListening && (
            <span className="ml-2 font-mono text-[10px] tracking-widest text-cyan/55">
              lvl {Math.round(audioLevel * 100).toString().padStart(2, "0")}
            </span>
          )}
        </motion.div>

        {voice.error && (
          <div className="font-mono text-[11px] text-signal-danger">
            mic: {voice.error}
          </div>
        )}
      </section>
      ...
      <section className="flex flex-col gap-4">
        ...
        >
          <button
            ...
            disabled={sending || voice.isProcessing}
            title="Hold to talk"
            aria-label="Hold to talk"
            className={`flex h-9 w-9 items-center justify-center rounded border transition select-none touch-none ${
              voice.isListening
                ? "border-cyan text-cyan shadow-glow-cyan"
                : "border-cyan/25 text-cyan/70 hover:border-cyan/60 hover:text-cyan"
          ...
          <input
            ...
            placeholder={voice.isListening ? "Listening…" : "Ask Apollo…"}
