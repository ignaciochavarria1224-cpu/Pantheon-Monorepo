...
| `Maridian` | Reflective journal-processing, cycle engine, and personal memory system for Apollo | `active/Pantheon/apps/maridian` | `canonical` | canonical Pantheon app at `active/Pantheon/apps/maridian`, canonical vault at `active/Pantheon/data/maridian-vault`, standalone material at `active/Maridian`, direct Obsidian access to the same vault | standalone README, repo-level Maridian vision, Maridian build plan, storage-path decision note, repo policy | migration execution milestones and privacy boundary decision | Pantheon-local ownership defined; storage path locked; control surface and migration still incomplete | journal ownership, Git/private boundary, and Pantheon tab planning still need implementation follow-through | define Maridian privacy boundary and plan the first Pantheon control-surface milestones |
