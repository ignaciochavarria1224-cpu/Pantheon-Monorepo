...
| `Maridian` | Reflective journal-processing, cycle engine, and personal memory system for Apollo | `active/Pantheon/apps/maridian` | `canonical` | canonical Pantheon app at `active/Pantheon/apps/maridian`, standalone material at `active/Maridian`, mirrored outputs in Obsidian | standalone README, repo-level Maridian vision, Maridian build plan | local storage-path decision note and migration execution milestones | Pantheon-local ownership defined; control surface and migration still incomplete | journaling ownership and local mirror flow still need implementation | build the Pantheon-native Maridian control surface and move journaling out of BlackBook |
...
2. local Maridian storage-path decision note
3. Maridian migration execution milestones
